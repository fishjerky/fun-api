...
class REST_Controller extends CI_Controller {
	...
	{	
		...
		{
			switch ($this->config->item('rest_auth'))
			{
				case "basic":
	...
	private function _prepare_oauth()
	{
		...
		//��oaccess token
		$access_token = trim($this->input->get_post("access_token"));
		...
		//�ˬdaccess token �O�_���T
		if(!$this->oauth_auth_server->validate_access_token($access_token))
		{
			header('HTTP/1.0 401 Unauthorized');
			header('HTTP/1.1 401 Unauthorized');
		}
		...
		//�ϥΪ�uid,username
		$user = $this->oauth_auth_server->get_user($access_token);

		if(empty($this->uid) || strtolower($this->uid)=='me'){
			$this->_args['uid'] = $user['uid'];
		}
		...
		//�v������
		if(strtolower($this->uid)!='me' && $this->uid != $this->_args['uid'])
		{
			header('HTTP/1.0 401 Unauthorized');
			header('HTTP/1.1 401 Unauthorized');
			exit;
